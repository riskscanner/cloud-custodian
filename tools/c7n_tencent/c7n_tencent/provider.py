...
from c7n_tencent.resources.resource_map import ResourceMap
from .client import Session

log = logging.getLogger('custodian.c7n_tencent')

@clouds.register('tencent')
class TencentCloud(Provider):

    display_name = 'Tencent'
    resource_prefix = 'tencent'
...
def _default_region(options):
    ...
    try:
        options.regions = [os.getenv('TENCENT_DEFAULT_REGION')]
    except Exception:
        log.warning('Could not determine default region')
        options.regions = [None]

    if options.regions[0] is None:
        log.error('No default region set. Specify a default via TENCENT_DEFAULT_REGION')
        sys.exit(1)

    log.debug("using default region:%s from TENCENT_DEFAULT_REGION" % options.regions[0])
