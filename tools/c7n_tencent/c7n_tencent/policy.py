...
class FunctionMode(ServerlessExecutionMode):
    ...
    def __init__(self, policy):
        self.policy = policy
        self.log = logging.getLogger('custodian.tencent.funcexec')
...
@execution.register('tencent-periodic')
    ...
    def validate(self):
        ...
        if 'tz' in mode:
            error = PolicyValidationError(
                "policy:%s tencent-periodic invalid tz:%s" % (
                    self.policy.name, mode['tz']))
            # We can't catch all errors statically, our local tz retrieval
            # then the form tencent is using, ie. not all the same aliases are
            # defined.
...
@execution.register('tencent-audit')
class ApiAuditMode(FunctionMode):
    """Custodian policy execution on tencent api audit logs events.
    ...
    logs creates an event for every api call that occurs in your tencent
    account. See `tencent Audit Logs
    ...
    def resolve_resources(self, event):
        """Resolve a tencent resource from its audit trail metadata.
        """
    ...
    def run(self, event, context):
        """Execute a tencent serverless model"""
